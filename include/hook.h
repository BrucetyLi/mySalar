...
// hook实际上就是对系统调用API进行一次封装，将其封装成一个与原始的系统调用API同名的接口，
//应用在调用这个接口时，会先执行封装中的操作，再执行原始的系统调用API。。就算没有HOOK模块也能完成所有的功能
...
// 使用dlsym找回被覆盖的符号时，第一个参数固定为 RTLD_NEXT，第二个参数为符号的名称.通过dlsym找到标准库中的符号地址
// 关于hook模块和IO协程调度的整合。一共有三类接口需要hook.sleep延时系列接口，包括sleep/usleep/nanosleep;
// socket IO系列接口，包括read/write/recv/send...等。socket/fcntl/ioctl/close等接口
namespace sylar {
    /**
     * @brief 当前线程是否hook
     */
