...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    hints.ai_flags = 0;  // 标志位（如 AI_PASSIVE）
    hints.ai_family = family; // 地址族（AF_INET, AF_INET6）
    hints.ai_socktype = type; // Socket 类型（SOCK_STREAM, SOCK_DGRAM）
    hints.ai_protocol = protocol; // 协议类型（IPPROTO_TCP, IPPROTO_UDP）
    hints.ai_addrlen = 0;  // 地址结构长度
    hints.ai_canonname = NULL; // 规范主机名
    hints.ai_addr = NULL; // 地址信息（如 sockaddr_in）
    hints.ai_next = NULL; // 链表下一个节点
    ...
    if(!host.empty() && host[0] == '[') {
        const char* endipv6 = (const char*)memchr(host.c_str() + 1, ']', host.size() - 1);// 若']'在该范围存在，返回其指针
        if(endipv6) {
    ...
    // getaddrinfo 的核心作用是将主机名和服务名转换为网络地址: 
    //1.将域名（如 www.example.com）或 IP 地址（如 192.168.1.1）解析为二进制地址格式。
    //2.将服务名（如 http）或端口号（如 80）转换为端口号。
    //3.结果是一个链表（addrinfo 结构），包含所有可能的地址组合（如 IPv4 + TCP、IPv6 + TCP 等）。
...
bool Address::GetInterfaceAddresses(std::vector<std::pair<Address::ptr, uint32_t> >&result
    ...
    auto its = results.equal_range(iface);  //在已排序的序列中查找所有等于指定值的元素，并返回一个迭代器范围（pair of iterators），表示这些元素的起始和结束位置
