...
void ByteArray::writeInt32  (int32_t value) {  // 都会先通过zegzag编码成uint在用varint序列化
...
void ByteArray::writeFloat  (float value) {  // 当成uint32存储
    uint32_t v; 
...
uint32_t ByteArray::readUint32() {  // varint反序列化
    uint32_t result = 0;
...
void ByteArray::read(void* buf, size_t size) {
    //判断要读的尺寸是否大于可读的尺寸，其他代码和write基本一致
    if(size > getReadSize()) {
...
void ByteArray::addCapacity(size_t size) {
    ...
    Node* tmp = m_root; //用临时指针指向根节点 准备开始遍历链表
    while(tmp->next) {
        tmp = tmp->next; //遍历链表，最终将临时指针 tmp 指向最后一个节点
    }

    Node* first = NULL;
    for(size_t i = 0; i < count; ++i) { //循环要增加的节点个数 准备添加新节点
        tmp->next = new Node(m_baseSize);
        if(first == NULL) { //并且记录下开始追加的首节点
            first = tmp->next;
        }
        tmp = tmp->next;
        m_capacity += m_baseSize; //将当前总容量重新计算，算上新追加的节点尺寸
    }

    if(old_cap == 0) {  //当我们扩容后应该要将当前操作的内存块指针指向 首个追加的节点上
        m_cur = first;
    }
}
