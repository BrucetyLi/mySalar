...
// 通知调度器有任务要调度, 写pipe让idle协程从epoll_wait退出，待idle协程yield之后Scheduler::run就可以调度其他任务
...
void IOManager::idle() {
    ...
    while(true) {
        ...
        if(SYLAR_UNLIKELY(stopping(next_timeout))) { // 这里的stopping会将 next_timeout设置成下一个timer的时间
        ...
        do {
            ...
            if(rt < 0 && errno == EINTR) { //EINTR仅表示等待被中断，不会丢失已就绪的事件。事件仍在epoll实例中，下次调用epoll_wait时会返回。
            } else { // 当 epoll_wait() 在阻塞等待 I/O 事件时，若进程收到一个信号（如 SIGALRM、SIGCHLD 或用户自定义信号），内核会中断 epoll_wait() 的阻塞状态，使其提前返回。
                break;
            }
        } while(true);
        ...
        for(int i = 0; i < rt; ++i) {
            ...
            if(event.data.fd == m_tickleFds[0]) {
                ...
                uint8_t dummy[256];
                while(read(m_tickleFds[0], dummy, sizeof(dummy)) > 0); //返回值 = 0：表示已到达文件末尾，且本次调用未读取到任何数据
                continue;
            }
